...
import { BarChart3, TrendingUp, Receipt, Wallet, Download, Filter, Package, CreditCard, FileJson, FileSpreadsheet } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const handleExportExcel = () => {
    ...
    sheets.push({
      ...
      rows: [
        ['Sales Revenue', Number(filteredRevenue) || 0],
        ['Bill Count', Number(filteredCount) || 0],
        ['Avg Bill', filteredCount > 0 ? filteredRevenue / filteredCount : 0],
        ['Total Expenses', Number(s.totalExpenses) || 0],
        ['Total Purchases', Number(s.totalPurchases) || 0],
        ['Net Profit', Number(filteredRevenue - s.totalExpenses - s.totalPurchases) || 0],
        ['Period Type', type],
    ...
    // Bills sheet (filtered)
    if (filteredBills.length > 0) {
      sheets.push({
        name: 'Bills',
        columns: ['Bill No', 'Date', 'Table', 'Items', 'Payment', 'Subtotal', 'Tax', 'Discount', 'Service', 'Total'],
        rows: filteredBills.map((b: any) => [
          ...
          Number(b.subtotal) || 0, Number(b.taxAmount) || 0,
          Number(b.discount) || 0, Number(b.serviceCharge) || 0, Number(b.total) || 0,
        ]),
      })
    }
    // Top items sheet
    const topItems = (filteredTopItems.length > 0 ? filteredTopItems : data.topItems || [])
    if (topItems.length > 0) {
      sheets.push({
        name: 'Top Items',
        columns: ['Item Name', 'Qty Sold', 'Revenue'],
        rows: topItems.map((it: any) => [it.name, Number(it.qty) || 0, Number(it.revenue) || 0]),
      })
    }
    ...
    if ((data.dailyBreakdown || []).length > 0) {
      sheets.push({
        ...
        columns: ['Date', 'Sales', 'Expenses'],
        rows: (data.dailyBreakdown || []).map((d: any) => [
          d.date, Number(d.sales || d.revenue || 0) || 0, Number(d.expenses || 0) || 0,
        ]),
      })
    }
    ...
    if (paymentEntries.length > 0) {
      sheets.push({
        ...
        rows: paymentEntries.map(([mode, v]: [string, any]) => [
          mode.toUpperCase(), Number(v.total || v) || 0, Number(v.count || 0) || 0,
        ]),
      })
    }
    ...
    downloadExcel(sheets, `report-${type}-${dateStr}`)
  }
