...
export const dashboard = {
  get(shopId: string) {
    ...
    return {
      ...
      topItems: topItems || [],
      lowStock: lowStock || [],
      cashFlow: { salesIn, otherIn, expenses, purchases, otherOut, net: salesIn + otherIn - expenses - purchases - otherOut },
      chartData,
    }
  },
}
...
export const reports = {
  get(shopId: string, from?: string, to?: string) {
    ...
    const bills = query<any>('SELECT * FROM Bill WHERE shopId = ? AND paidAt >= ? AND paidAt <= ? ORDER BY paidAt DESC', [shopId, fromIso, toIso])
    ...
    const salesRevenue = bills.reduce((s, b) => s + (b.total || 0), 0)
    ...
    for (const b of bills) byPayment[b.paymentMode] = (byPayment[b.paymentMode] || 0) + (b.total || 0)
    const topItemsMap: Record<string, { name: string; qty: number; revenue: number }> = {}
    for (const b of bills) {
      try {
        const items = JSON.parse(b.items || '[]')
        for (const it of items) {
          if (!topItemsMap[it.name]) topItemsMap[it.name] = { name: it.name, qty: 0, revenue: 0 }
          topItemsMap[it.name].qty += (it.quantity || 0)
          topItemsMap[it.name].revenue += (it.quantity || 0) * (it.price || 0)
        }
      } catch { /* ignore */ }
    }
    const topItems = Object.values(topItemsMap).sort((a, b) => b.qty - a.qty).slice(0, 10)
    ...
    for (const b of bills) {
      ...
      if (day) dailyMap[day] = (dailyMap[day] || 0) + (b.total || 0)
    }
    const dailyBreakdown = Object.entries(dailyMap).map(([date, revenue]) => ({ date, revenue })).sort((a, b) => a.date.localeCompare(b.date))
    return {
      summary: {
        salesRevenue, totalExpenses, totalPurchases,
        netProfit: salesRevenue - totalExpenses - totalPurchases,
        cashFlow: salesRevenue - totalExpenses - totalPurchases,
        billCount: bills.length,
        avgBill: bills.length ? salesRevenue / bills.length : 0,
      },
      byPayment, topItems, expenseByCategory, dailyBreakdown, bills,
    }
  },
}
